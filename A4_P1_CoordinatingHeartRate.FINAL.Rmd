...
```{r}
setwd("~/Documents/RStudioDocs/Assignment4/Portfolio4")
library(pacman)
...
m = lmer(RR.HR ~ type + (1|group), final_df)
summary(m)
r.squaredGLMM(m)

m2 = lmer(RR.Resp ~ type  + (1|group), final_df)
summary(m2)
r.squaredGLMM(m2)
```
...
```{r}
m = lmer(RR.HR ~ condition + (1|group), og_df)
summary(m)

m2 = lmer(RR.Resp ~ condition + (1|group), og_df)
summary(m2)
r.squaredGLMM(m2)

# heart rate models in type 
...
summary(mNRLINE) # NOT SIGGGGGGG (error?? )
summary(mENTR) #yas
summary(mrENTR) #yas
summary(mLAM) #helt sikkert


# heart rate in condition 
...
# respiration models for condition 
